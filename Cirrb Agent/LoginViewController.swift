    ...
    private func loginUser(email: String, password: String, device_token: String) {
        self.appDel.apiManager.login(email: email, password: password, device_token: device_token, onComplete: {
            ...
            if details != nil{
                ...
                let Cash_in_hand = detailDict.value(forKey: "cash_in_hand") as! Int
                ...
                if (auth_token.trimmingCharacters(in: .whitespaces)).characters.count != 0 {
                    ...
                    UserDefaults.standard.setValue(phone, forKey: Constant.User.PHONE)
                    ...
                    UserDefaults.standard.setValue(String(describing: Cash_in_hand), forKey: Constant.User.LIMIT_LEFT)
                    
                    UserDefaults.standard.setValue(String(order_limit), forKey: Constant.User.ORDER_LIMIT)
                    
//                    let x = UserDefaults.standard.object(forKey: "Search_VC") as? String
//             
//                    if x != nil{
//                    self.performSegue(withIdentifier: "Login_search", sender: self)
//                    }else{
